    ...
    /* 1. Tự động giữ thanh cuộn ở đầu trang vì log mới nhất hiện ở trên cùng */
    const chatArea = document.getElementById('chatArea');
    if (chatArea) {
        chatArea.scrollTop = 0;
    }

    // 2. Hàm đổ dữ liệu động và kích hoạt căn giữa tuyệt đối
    function showUserDetailFromDataset(element) {
        const d = element.dataset;
        
        document.getElementById('log-m-username').textContent = '@' + d.username;
        document.getElementById('log-m-fullname').textContent = d.fullname;
        document.getElementById('log-m-email').textContent = d.email;
        document.getElementById('log-m-birth').textContent = d.birth;
        document.getElementById('log-m-phone').textContent = d.phone;
        
        const roleEl = document.getElementById('log-m-role');
        roleEl.textContent = d.role;
        roleEl.className = d.role.includes('Quản trị viên') ? 'badge bg-danger text-light' : 'badge bg-danger text-light';
        roleEl.style.padding = '4px 8px';

        const statusEl = document.getElementById('log-m-status');
        statusEl.textContent = d.status;
        statusEl.className = d.status.includes('Đang hoạt động') ? 'badge bg-success text-light' : 'badge bg-success text-light';
        statusEl.style.padding = '4px 8px';

        const modalElement = document.getElementById('userLogDetailModal');
    
    // Sử dụng Bootstrap API chuẩn
    if (typeof bootstrap !== 'undefined') {
        const myModal = bootstrap.Modal.getOrCreateInstance(modalElement);
        myModal.show();
    } else {
        // Fallback thủ công nếu Bootstrap không tải
 ...
 document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateWheelNumbers(wheelNode, totalCount) {
        ...
        // Kích hoạt kéo chuột trên PC
        enableMouseDragScroll(wheelNode);

        // =========================================================================
        // KHỐI THÊM MỚI: CHỐNG ĐỨNG HÌNH TUYỆT ĐỐI VÀ GIỮ HIỆU ỨNG CUỘN IPHONE MƯỢT
        // =========================================================================
        let isWheelAnimating = false;
        wheelNode.addEventListener('wheel', function(e) {
            e.preventDefault(); // Chặn đứng hành vi cuộn mặc định của trình duyệt để không bao giờ bị đơ

            if (isWheelAnimating) return; // Đang chạy animation mượt thì bỏ qua các góc lăn nhỏ dồn dập

            const rowHeight = 30; // Chiều cao 1 dòng số
            const direction = e.deltaY > 0 ? 1 : -1; // 1 là xuống, -1 là lên
            
            // Tính toán vị trí đích chuẩn khấc tiếp theo
            let targetScroll = wheelNode.scrollTop + (direction * rowHeight);

            // Kiểm tra biên an toàn (Giới hạn cuộn không cho vượt quá danh sách số hiện có)
            const maxScroll = (totalCount * rowHeight) - rowHeight;
            if (targetScroll < 0) targetScroll = 0;
            if (targetScroll > maxScroll) targetScroll = maxScroll;

            isWheelAnimating = true;
            wheelNode.style.scrollSnapType = 'none'; // Tắt tạm snap để scrollTo chạy mượt

            wheelNode.scrollTo({
                top: targetScroll,
                behavior: 'smooth' // Tạo hiệu ứng lướt mượt mà chuẩn iOS
            });

            // Sau khi hiệu ứng trượt kết thúc (80ms), bật lại chế độ hít (snap) số
            setTimeout(() => {
                wheelNode.style.scrollSnapType = 'y mandatory';
                isWheelAnimating = false;
            }, 80); 
        }, { passive: false }); // Bắt buộc là false để e.preventDefault() có hiệu lực
    }
